...
```{r}
df_k %>%  group_by(treatmentgroup) %>% summarize( nothing = sum(inequality == 1), n=n())
prop.test(c0$y0[c0$treatmentgroup %in% c("Forced Choice", "Nominal Choice") ], 
          c0$n[c0$treatmentgroup %in% c("Forced Choice", "Nominal Choice") ])
```
